  ...
  it("covers think status and /thinking xhigh support matrix", async () => {
    await withTempHome(async (home) => {
      ...
      expect(unsupportedModelTexts).toContain(
        'Thinking level "xhigh" is only supported for openai/gpt-5.4, openai/gpt-5.2, openai-codex/gpt-5.4, openai-codex/gpt-5.3-codex, openai-codex/gpt-5.3-codex-spark, openai-codex/gpt-5.2-codex, openai-codex/gpt-5.1-codex, github-copilot/gpt-5.2-codex or github-copilot/gpt-5.2.',
      );
      expect(runEmbeddedPiAgent).not.toHaveBeenCalled();
    });
  });
