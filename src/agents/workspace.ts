...
export const DEFAULT_PEERS_FILENAME = "PEERS.md";
...
export async function ensureAgentWorkspace(params?: {
  ...
  const peersPath = path.join(dir, DEFAULT_PEERS_FILENAME);
  ...
  const peersTemplate = await loadTemplate(DEFAULT_PEERS_FILENAME);
  ...
  await writeFileIfMissing(peersPath, peersTemplate);
...
export async function loadWorkspaceBootstrapFiles(dir: string): Promise<WorkspaceBootstrapFile[]> {
  ...
  const entries: Array<{
    ...
    },
    {
      ...
      filePath: path.join(resolvedDir, DEFAULT_PEERS_FILENAME),
    },
    {
